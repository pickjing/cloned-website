...
class DtuData {
  ...
  static async get(params = {}, connection = null) {
    ...
    const { dtuId, page = 1, limit = 20, group, status, search } = params;
  ...
  static async delete(data, connection = null) {
    ...
    // 检查DTU设备是否存在
    const [existingRows] = await dbConnection.execute(
      'SELECT dtu_id, dtu_name FROM dtu_devices WHERE dtu_id = ?',
      [dtu_id]
    );
    ...
    // 删除DTU设备（级联删除相关的传感器和MB-RTU配置）
    const [result] = await dbConnection.execute(
      'DELETE FROM dtu_devices WHERE dtu_id = ?',
      [dtu_id]
    );
